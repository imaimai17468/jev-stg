...
describe(partsOf, () => {
  it("should put each government to Jev as a council of its own when the council meets", () => {
    const second: NationBrief = { ...BRIEF, nation: 2 };

    expect(partsOf({ ...COUNCIL, nations: [BRIEF, second] })).toStrictEqual([
      { ...COUNCIL, nations: [BRIEF] },
      { ...COUNCIL, nations: [second] },
    ]);
  });

  it("should put peace talks to Jev as they are when a nation has surrendered", () => {
    expect(partsOf(TALKS)).toStrictEqual([TALKS]);
  });
});
