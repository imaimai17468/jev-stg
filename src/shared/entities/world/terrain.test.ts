...
describe(terrainAt, () => {
  it("should be mountains when the height is above the mountain line", () => {
    expect(terrainAt(0.9, 0.5, 0.1)).toBe("mountains");
  });

  it("should be hills when the height clears the hill line but not the mountain one", () => {
    expect(terrainAt(0.7, 0.5, 0.1)).toBe("hills");
  });

  it("should be tundra when the lowland sits near a pole", () => {
    expect(terrainAt(0.5, 0.5, 0.9)).toBe("tundra");
  });

  it("should be desert when the lowland is dry", () => {
    expect(terrainAt(0.5, 0.2, 0.1)).toBe("desert");
  });

  it("should be forest when the lowland is wet", () => {
    expect(terrainAt(0.5, 0.7, 0.1)).toBe("forest");
  });

  it("should be plains when the lowland is neither dry nor wet", () => {
    expect(terrainAt(0.5, 0.45, 0.1)).toBe("plains");
  });
});
